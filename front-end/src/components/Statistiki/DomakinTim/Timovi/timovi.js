...
const Timovi = (props) => {
    ...
    let [imeState, setImeState] = useState("Селектирај тим за кој што сакаш да се прикажат победите во градот во кој е основан!");
    ...
    let items = tState.map((tim, index) => {
        return <Dropdown.Item eventKey={`${index}`} onClick={prikazhiPobedi} imetim={tim.imeTim} idtim={tim.idTim}>{tim.imeTim} - {tim.grad.imeGrad}</Dropdown.Item>;
    });
