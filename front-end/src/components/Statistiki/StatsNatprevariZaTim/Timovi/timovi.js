...
const Timovi = (props) => {
    ...
    let [imeState, setImeState] = useState("Селектирај тим за кој што сакаш да се прикажат статистики!");
    ...
    const prikazhiStatistiki = (e) => {
        let idTim = e.target.attributes.getNamedItem("idtim").value;
        setImeState(e.target.attributes.getNamedItem("imetim").value);
        props.prikazhiStats(idTim);
    };

    let items = tState.map((tim, index) => {
        return <Dropdown.Item eventKey={`${index}`} onClick={prikazhiStatistiki} imetim={tim.imeTim} idtim={tim.idTim}>{tim.imeTim} - {tim.grad.imeGrad}</Dropdown.Item>;
    });
